...
Conectar o ServiceNow ao Cloud App Security oferece informações aprimoradas sobre as atividades dos usuários, fornece detecção de ameaças usando detecções de anomalias baseadas em aprendizado de máquina e detecções de proteção de informações, como identificar quando clientes confidenciais as informações são carregadas para a nuvem do ServiceNow.
